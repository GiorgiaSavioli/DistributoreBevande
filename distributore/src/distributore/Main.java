		...
		while (true) {
			...
			if (scelta == 1234) {
				...
				if (scelta < 1 || scelta > distributore.getProdotti().size())
					...
					double denaro = 0;
					System.out.println("Hai scelto " + distributore.getProdotti(scelta).getNome() + " il prezzo è di "
							+ distributore.getProdotti(scelta).getPrezzo() + " €");
					...
					double resto = distributore.verificaResto(distributore.getProdotti(scelta).getPrezzo(), denaro);

					if (distributore.getProdotti(scelta).getCaldo()) {
